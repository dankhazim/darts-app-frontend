...
export default function useThemeCustomizer() {
  ...
  const _loadStateFromProps = useCallback(() => {
    setDisableLayoutWidth(true);
    setDisableSidebarTheme(true);
    setDisableSidebarType(true);
  }, []);
  ...
  const changeLeftSidebarTheme = (value: string) => {
    ...
    switch (theme) {
      case 'default':
        dispatch(
          changeSidebarTheme(
            layoutConstants.SideBarTheme.LEFT_SIDEBAR_THEME_DEFAULT
          )
        );
        break;
      case 'light':
  ...
  const changeLeftSiderbarType = (value: string) => {
    ...
    switch (type) {
      ...
      case 'scrollable':
        dispatch(
          changeSidebarType(
            layoutConstants.SideBarWidth.LEFT_SIDEBAR_TYPE_SCROLLABLE
          )
        );
        break;
      default:
